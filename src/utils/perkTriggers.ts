...
export function triggerOnDamageTaken(amount: number, position: Position, ashChargeConsumed = false) {
  const activePerks = useDeckStore.getState().activePerks

  // Grease Fire
  const gfStacks = activePerks.find((p) => p.id === 'grease_fire')?.stackCount ?? 0
  if (gfStacks > 0) {
    const tier = Math.min(gfStacks, 3)
    const baseCd = [3.0, 2.5, 2.0][tier - 1]
    const cd = Math.max(0.5, baseCd - 0.2 * Math.max(0, gfStacks - tier))
    const now = performance.now() / 1000
    if (now - lastGreaseFireAt >= cd) {
      lastGreaseFireAt = now

      const baseDmg = [15, 25, 40][tier - 1] + 8 * Math.max(0, gfStacks - tier)
      ...
      const gfEnemies = useEnemyStore.getState().enemies
      const dmgColor = dmg >= 80 ? '#ef4444' : dmg >= 40 ? '#fbbf24' : '#ffffff'
      let hitCount = 0
      for (const enemy of gfEnemies) {
      ...
      const gfDef = PERK_POOL.find((p) => p.id === 'grease_fire')
      if (gfDef?.vfxSprite) {
        spawnSpriteVfx(gfDef.vfxSprite, position.x, position.z, radius * 2.5)
      } else {
  ...
  // Ashen Apron T2+ burst — fires when a charge was just consumed this hit
  const apron = activePerks.find((p) => p.id === 'ashen_apron')
  const apronTier = apron ? Math.min(apron.stackCount, 3) : 0
  if (apron && apronTier >= 2 && ashChargeConsumed) {
    const burstDamage = apronTier === 2 ? 15 : 25
    const burstRadius = apronTier === 2 ? 3 : 5
    const ashDef = PERK_POOL.find((p) => p.id === 'ashen_apron')
    if (ashDef?.vfxSprite) {
      spawnSpriteVfx(ashDef.vfxSprite, position.x, position.z, burstRadius * 2)
    } else {
      spawnExplosionVfx(position.x, position.z)
    }
    const ashEnemies = useEnemyStore.getState().enemies
    for (const enemy of ashEnemies) {
      if (enemy.dying || enemy.detonating) continue
      const dx = enemy.position.x - position.x
      const dz = enemy.position.z - position.z
      if (dx * dx + dz * dz > burstRadius * burstRadius) continue
      useEnemyStore.getState().damageEnemy(enemy.id, burstDamage)
      useEnemyStore.getState().setEnemyHitFlash(enemy.id, performance.now() + 100)
      spawnDamageNumberVfx(enemy.position.x, enemy.position.z, burstDamage)
      if (apronTier >= 3) {
        useEnemyStore.getState().setEnemySoaked(enemy.id, performance.now() + 1500)
      }
      const updated = useEnemyStore.getState().enemies.find((e) => e.id === enemy.id)
      if (!updated || updated.hp > 0) continue
      if (updated.type === 'exploder') {
