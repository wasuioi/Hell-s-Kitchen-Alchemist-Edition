...
0. **Sync local `main` from origin** (must run before any file read). Cron may fire before today's overnight changes have been pulled locally; without this step the task reads stale config files and fails. Run `git -C /Users/pacas/Hell-s-Kitchen-Alchemist-Edition fetch origin main`, verify HEAD is on `main` (fail loudly otherwise), then `git -C ... pull --ff-only origin main`. If either of those fails, **FAIL FAST**: print the exact stderr and EXIT THE TASK. Do NOT run any further git, shell, or other tool calls; do NOT investigate, probe, or attempt recovery. The user sees the failure via `notifyOnCompletion` and decides how to clean up local state. (This rule exists because, on 2026-04-29, the 12:09 quick-win run hit a pull conflict and spent 85 minutes probing instead of exiting; the LLM kept investigating because earlier wording said only "stop." "FAIL FAST → EXIT" is the contract.)
